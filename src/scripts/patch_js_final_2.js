...
if (s !== -1 && e !== -1) {
    const newLogic = `window.onBulkTargetChange = function() {
        ...
        window.populateUnidadDropdown = function() {
            ...
            // forEachNode barre ABSOLUTAMENTE TODO el Client-Side Row Model (ignora filtros vivos de AG-Grid)
                ...
                if (node.data && node.data.datos_maestros) {
                    ...
                    for (let key in dm) {
                        if (String(key).toLowerCase() === 'unidad') {
                            const rawVal = dm[key];
                            // Exclusión rígida de vacíos: null, undefined, '', y el rótulo "[vacio]" o espacios muertos
    ...
    console.log('JS fully updated with safe conversion constraints');
} else {
