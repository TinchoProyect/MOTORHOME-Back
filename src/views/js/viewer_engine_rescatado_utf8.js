...
/**
 * VIEWER ENGINE - Sistema de Gesti├│n de Proveedores
 * M├│dulo de Visualizaci├│n, Worker Excel y Herramientas de Mapeo
 * v2.6 (Regex Logic Restored + Computed Columns + Preview Fix)
 */

console.log("%c ­ƒÜÇ VIEWER ENGINE: v2.6 - READY ", "background: #8b5cf6; color: #fff; font-weight: bold; padding: 4px;");

// --- 1. VARIABLES GLOBALES (Scope M├│dulo) ---
// --- 1. VARIABLES GLOBALES (MIGRADO A viewer_core.js) ---
// viewerWorker, currentSheetData, workbook, etc. ahora residen en viewer_core.js


// --- 2. C├ôDIGO DEL OBRERO (Worker) ---
// --- 2. C├ôDIGO DEL OBRERO (MIGRADO A viewer_worker.js) ---
        ...
        if (isExcel) {
            ...
            const initWatchdog = setTimeout(() => {
                if (useWorker && !currentWorkbook) {
                    console.error("­ƒÜ¿ Worker INIT Timeout (7s).");
                    useWorker = false;
                    viewerWorker.terminate();
            ...
            viewerWorker.onerror = (e) => {
                ...
                console.error("­ƒÜ¿ Worker Crash:", e);
                useWorker = false;
                e.preventDefault();
                viewerWorker.terminate();
...
const processLocally = (wb, sName) => {
    console.warn("ÔÜá´©Å Fallback Local activado para: " + sName);
    const excelContainer = document.getElementById('excelContainer');
    ...
    try {
        ...
        const errContainer = document.getElementById('errorContainer');
        if (errContainer) {
            errContainer.textContent = "Error Cr├¡tico: " + e.message;
            errContainer.classList.remove('hidden');
        }
    }
};
...
async function loadNomenclature() {
    try {
        ...
        if (nomenclatureCache.length === 0) {
            nomenclatureCache = [
                { id: 'temp1', termino: 'C├│digo', descripcion_uso: 'SKU' },
                { id: 'temp2', termino: 'Descripci├│n', descripcion_uso: 'Nombre' },
...
async function updateNomenclatureTerm(id, newTerm, newDesc, newRules) {
    try {
        ...
        if (idx !== -1) {
            ...
            // ­ƒöÑ HOT RELOAD FIX (Actualiza memoria del visor al guardar)
...
function createTermSelect(currentId, placeholder, currentTermId) {
    ...
    if (currentId && !foundCurrent) {
        ...
        legacyOpt.text = `${displayName} (Inv├ílido)`;
...
function renderEditMode(container, term) {
    ...
    header.innerHTML = '<span class="text-[9px] font-bold text-blue-400 uppercase tracking-widest">Edici├│n R├ípida</span>';
    container.appendChild(header);

    // Input: Name
    const group1 = document.createElement('div');
    group1.className = "space-y-1";
    group1.innerHTML = '<label class="text-[9px] text-slate-500 uppercase font-bold">T├®rmino</label>';
    ...
    group2.innerHTML = '<label class="text-[9px] text-slate-500 uppercase font-bold">Descripci├│n</label>';
...
function applyProcessingRules(originalData) {
    ...
    const processedData = [];
    // ­ƒöÑ SET PARA DEDUPLICAR (Memoria de Elefante)
            ...
            if (rule.type === 'sanitize') {
                ...
                if (!shouldReplace && rule.config?.match_regex) {
                    try {
                        ...
                        p = p.replace(/\\\\/g, '\\'); // ­ƒöÑ FIX v2.6 (DOBLE ESCAPE RESTAURADO)
                ...
                if (shouldReplace) {
                    ...
                    // Si es un n├║mero v├ílido pero tiene punto, lo pasamos a coma
            ...
            if (keepRow && (rule.type === 'split' || rule.type === 'regex_split')) {
                ...
                if (rule.type === 'regex_split') {
                    try {
                        ...
                        if (patternStr) {
                            patternStr = patternStr.replace(/\\\\/g, '\\'); // ­ƒöÑ FIX v2.6 (DOBLE ESCAPE RESTAURADO)
                ...
                if (pPres) {
                    row[colIdx] = `­ƒôª ${pDesc}  |  ­ƒÅÀ´©Å ${pPres}`;
                }
            }
...
function renderVirtualTable(originalData) {
    // ­ƒöÑ MODO MUERTITO: Tabla principal est├ítica (RAW)
    const data = originalData;

    // ­ƒöÑ STATE EXPOSURE FOR SATELLITE MODULES (v2.5)
    window.viewerState = { mapping: columnMapping, data: currentSheetData };

    const container = document.getElementById('excelContainer');

    if (!data || data.length === 0) {
        if (container) container.innerHTML = '<div class="text-slate-500 p-4">Hoja vac├¡a o todos los datos fueron filtrados.</div>';
        return;
    }
    ...
    try {
        ...
        if (modalTitle) {
            modalTitle.innerHTML = `
                <span class="text-slate-400 font-normal">Vista Previa de Extracci├│n:</span> 
        ...
        Object.keys(columnMapping).forEach(key => {
            ...
            if (termId && termId !== 'Ignorar Columna') {
                ...
                if (rule && isSimActive && rule.type === 'split') {
                    ...
                    let patternStr = rule.pattern;
                    if (patternStr) {
                        patternStr = patternStr.replace(/\\\\/g, '\\'); // ­ƒöÑ FIX v2.6 (Double backslash restored)
                        if (patternStr.startsWith('/')) patternStr = patternStr.slice(1);
                        if (patternStr.endsWith('/i')) patternStr = patternStr.slice(0, -2);
                        else if (patternStr.endsWith('/')) patternStr = patternStr.slice(0, -1);
        ...
        // ­ƒöÑ FILTRADO REAL + ANTI-DUPLICADOS (REGEX FIX v2.6)
        const seenValues = new Set();
        ...
        document.getElementById('simMeta').innerHTML = `
            <span class="text-slate-400">Total Filas ├Ütiles:</span> <span class="text-white font-bold">${sanitizedData.length}</span> 
        ...
        return terms.every(term => {
            if (fieldIdx === "ALL") {
                return currentDisplayConfig.some(cfg => {
                    const val = cfg.transform(row[cfg.sourceIndex], row); // ­ƒöÑ PASS ROW (v2.5)
...
function renderSimulationTable(data) {
    ...
    data.forEach((row) => {
        ...
        currentDisplayConfig.forEach(cfg => {
            ...
            const finalVal = cfg.transform(rawVal, row); // ­ƒöÑ PASS ROW (v2.5)
...
window.loadVirtualWorkbook = function (workbookMap, fileName) {
    ...
    // 5. Load First Sheet (Delegado a loadSheet para conversi├│n correcta)
    loadSheet(sheetNames[0]);
};
...
console.log("Ô£à VIEWER ENGINE INITIALIZED & EXPOSED");
